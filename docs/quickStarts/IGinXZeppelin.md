...
构建成功后，在`IGinX/zeppelin-interpreter/target/`路径下找到`zeppelin-iginx-0.7.2-jar-with-dependencies.jar`包。
...
在`zeppelin-0.10.1-bin-netinst/interpreter/`文件夹下新建一个文件夹`IGinX`，将构建好的`zeppelin-iginx-0.7.2-jar-with-dependencies.jar`包放入其中即可。
...
将`zeppelin-iginx-0.7.2-jar-with-dependencies.jar`包放入我们准备好的`zeppelin-interpreter`文件夹内即可。
