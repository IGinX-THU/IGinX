...
public abstract class MigrationPolicy {
    ...
    public void reshardByCustomizableReplica(
        ...
        try {
            ...
            String currStartTimeseries = fragmentMeta.getColumnsRange().getStartColumn();
            long currLoad = 0L;
            String endTimeseries = fragmentMeta.getColumnsRange().getEndColumn();
            ...
            while (fakedFragmentMetas.size() > maxReshardFragmentsNum) {
                ...
                for (int i = 0; i < fakedFragmentMetaLoads.size(); i++) {
                    ...
                    if (fakedFragmentMetaLoads.get(i)
                            ...
                            && currFragmentMeta
                                    .getColumnsRange()
                                    .getStartColumn()
                                    .equals(currFragmentMeta.getColumnsRange().getEndColumn())) {

                        // 与他最近的负载最低的时间分区进行合并
                        if (i == (fakedFragmentMetaLoads.size() - 1)
                                || fakedFragmentMetaLoads.get(i + 1)
                                        > fakedFragmentMetaLoads.get(i - 1)) {
                            ...
                            fakedFragmentMetaLoads.set(
                                    i - 1,
                                    fakedFragmentMetaLoads.get(i - 1)
                                            + fakedFragmentMetaLoads.get(i));
                            fakedFragmentMetaLoads.remove(i);
                        } else if (fakedFragmentMetaLoads.get(i + 1)
                                <= fakedFragmentMetaLoads.get(i - 1)) {
                ...
                if (canMergeFragments) {
                    ...
                    fakedFragmentMetaLoads.set(
                            startIndex,
                            fakedFragmentMetaLoads.get(startIndex)
                                    + fakedFragmentMetaLoads.get(startIndex + 1));
            ...
            for (int i = 0; i < fakedFragmentMetas.size(); i++) {
                ...
                if (i == 0) {
                    ...
                    FragmentMeta newFragment =
                            new FragmentMeta(
                                    targetFragmentMeta.getColumnsRange().getStartColumn(),
                                    targetFragmentMeta.getColumnsRange().getEndColumn(),
                ...
                if (fakedFragmentMetaLoads.get(i)
                    ...
                    for (int num = 1; num < replicas; num++) {
                        long targetStorageId =
                                storageHeatEntryList
                                        .get(num % storageHeatEntryList.size())
                                        .getKey();
                        StorageUnitMeta newStorageUnitMeta =
                                DefaultMetaManager.getInstance()
                                        .generateNewStorageUnitMetaByFragment(
                                                fragmentMeta, targetStorageId);
                        FragmentMeta newFragment =
                                new FragmentMeta(
                                        targetFragmentMeta.getColumnsRange().getStartColumn(),
                                        targetFragmentMeta.getColumnsRange().getEndColumn(),
        ...
        migrationTaskQueue.sort(
                (o1, o2) -> {
                    ...
                    if (migrationTask1 == null || migrationTask2 == null) {
                        ...
                        return (int)
                                (migrationTask2.getPriorityScore()
                                        - migrationTask1.getPriorityScore());
                    }
                });
    }
                        ...
                        () -> {
                            ...
                            if (migrationTask.getMigrationType() == MigrationType.QUERY) {
                                ...
                                if (migrationTask.getFragmentMeta().getKeyInterval().getEndKey()
                                        == Long.MAX_VALUE) {
                                    this.logger.error(
                                            "start to reshard query data: {}", migrationTask);
                                    FragmentMeta fragmentMeta =
                                            reshardFragment(
            ...
            while (rowStream.hasNext()) {
                ...
                for (FragmentMeta fragmentMeta : fragmentMetas) {
                    if (fragmentMeta.getColumnsRange().isContain(timeSeries)) {
                        pathSet.add(timeSeries);
                        logger.info(
                                "[migrationData] path {} belong to {}", timeSeries, fragmentMeta);
                    }
                }
            }
