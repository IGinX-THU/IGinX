...
public class DataPointsParser {
    ...
    // 修改路径，并插入数据
    public void handleAnnotationAppend(Query preQuery, QueryResult preQueryResult)
            throws Exception {
        // 创建session
        try {
            session.openSession();
            ...
            for (int pos = 0;
                ...
                for (int pl = 0; pl < queryResultDataset.getPaths().size(); pl++) {
                    ...
                    String name =
                            pathAppendAnno(
                                    metric,
                                    queryResultDataset.getPaths().get(pl),
                                    queryBase.getAnnotationLimit());
                    metric.setName(name);
            ...
            for (int pos = 0;
                ...
                for (int pl = 0; pl < queryResultDataset.getPaths().size(); pl++) {
                    ...
                    Map<String, String> tags =
                            getTagsFromPaths(queryResultDataset.getPaths().get(pl), name);
                    ...
                    metricGetData(
                            ...
                            queryBase,
                            queryResultDataset,
                            queryBase.getNewAnnotationLimit(),
                            pl);
