...
public class InsertGenerator extends AbstractGenerator {
  ...
  protected Operator generateRoot(Statement statement) {
    ...
    Map<TimeSeriesRange, List<FragmentMeta>> fragments =
        metaManager.getFragmentMapByTimeSeriesIntervalAndTimeInterval(tsInterval, timeInterval);
  ...
  private DataView getDataSection(FragmentMeta meta, RawData rawData) {
    ...
    if (tsInterval.getStartTimeSeries() != null
        && tsInterval.getStartTimeSeries().compareTo(paths.get(paths.size() - 1)) > 0) return null;
    ...
    if (tsInterval.getEndTimeSeries() != null) {
      while (endPathIndex < paths.size()
          && tsInterval.getEndTimeSeries().compareTo(paths.get(endPathIndex)) > 0) endPathIndex++;
    } else {
