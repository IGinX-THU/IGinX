...
public class ZooKeeperMetaStorage implements IMetaStorage {
    ...
    public ZooKeeperMetaStorage() {
        ...
        timeseriesHeatCounterMutex =
                new InterProcessMutex(client, TIMESERIES_HEAT_COUNTER_LOCK_NODE);
    }
        ...
        try {
            ...
            try {
                ...
                throw new MetaStorageException(
                        "get error when release interprocess lock for " + SCHEMA_MAPPING_LOCK_NODE,
                        e);
            }
        }
    }
    ...
    private void registerSchemaMappingListener() throws Exception {
        ...
        TreeCacheListener listener =
                (curatorFramework, event) -> {
                    ...
                    String schema =
                            event.getData().getPath().substring(SCHEMA_MAPPING_PREFIX.length());
        ...
        try {
            ...
            if (this.client.checkExists().forPath(SCHEMA_MAPPING_PREFIX + "/" + schema) == null) {
                ...
                this.client
                        ...
                        .forPath(
                                SCHEMA_MAPPING_PREFIX + "/" + schema,
                                JsonUtils.toJson(schemaMapping));
            } else {
    ...
    public Map<Long, StorageEngineMeta> loadStorageEngine(List<StorageEngineMeta> storageEngines)
        ...
        try {
            ...
            try {
                ...
                throw new MetaStorageException(
                        "get error when release interprocess lock for " + STORAGE_ENGINE_LOCK_NODE,
                        e);
            }
        }
    }
    ...
    public boolean updateStorageEngine(long storageID, StorageEngineMeta storageEngine)
        ...
        try { // node0000000002 STORAGE_ENGINE_NODE_NUM_LENGTH
            ...
            String tmp = new String(JsonUtils.toJson(storageEngine));
            String nodeName =
                    generateID(STORAGE_ENGINE_NODE, STORAGE_ENGINE_NODE_NUM_LENGTH, storageID);
            this.client.setData().forPath(nodeName, tmp.getBytes());
            return true;
        } catch (Exception e) {
                        ...
                        case NODE_UPDATED:
                            ...
                            if (storageEngineMeta != null) {
                                ...
                                storageChangeHook.onChange(
                                        storageEngineMeta.getId(), storageEngineMeta);
                            } else {
        ...
        try {
            ...
            if (this.client.checkExists().forPath(STORAGE_UNIT_NODE_PREFIX) == null) {
                ...
                this.client
                        .create()
                        .withMode(CreateMode.PERSISTENT)
                        .forPath(STORAGE_UNIT_NODE_PREFIX);
            } else {
                List<String> storageUnitIds =
                        this.client.getChildren().forPath(STORAGE_UNIT_NODE_PREFIX);
                ...
                for (String storageUnitId : storageUnitIds) {
                    ...
                    logger.error(storageUnitMeta.getId());
                    logger.error(storageUnitMeta.getMasterId());
                    logger.error(String.valueOf(storageUnitMeta.isInitialStorageUnit()));
                    logger.error(String.valueOf(storageUnitMeta.getCreatedBy()));
                    logger.error(String.valueOf(storageUnitMeta.isDummy()));
                    logger.error(String.valueOf(storageUnitMeta.isIfValid()));
                    logger.error(String.valueOf(storageUnitMeta.isMaster()));
                    logger.error(String.valueOf(storageUnitMeta.getStorageEngineId()));
                    ...
                    switch (event.getType()) {
                        ...
                        case NODE_UPDATED:
                            ...
                            if (storageUnitMeta != null) {
                                logger.info(
                                        "new storage unit comes to cluster: id = "
                                                + storageUnitMeta.getId());
                                storageUnitChangeHook.onChange(
                                        storageUnitMeta.getId(), storageUnitMeta);
                            }
                            break;
                        default:
    ...
    public List<FragmentMeta> getFragmentListByTimeSeriesNameAndTimeInterval(
        ...
        try {
            List<String> columnsRangeNames =
                    this.client.getChildren().forPath(FRAGMENT_NODE_PREFIX);
            for (String columnsRangeName : columnsRangeNames) {
                ...
                if (fragmentTimeSeries.isContain(tsName)) {
                    ...
                    List<String> timeIntervalNames =
                            this.client
                                    .getChildren()
                                    .forPath(FRAGMENT_NODE_PREFIX + "/" + columnsRangeName);
    ...
    public void updateFragmentByColumnsRange(ColumnsRange columnsRange, FragmentMeta fragmentMeta)
        ...
        try {
            ...
            List<String> timeIntervalNames =
                    this.client
                            .getChildren()
                            .forPath(FRAGMENT_NODE_PREFIX + "/" + columnsRange.toString());
                ...
                (curatorFramework, event) -> {
                    ...
                    switch (event.getType()) {
                        ...
                        case NODE_UPDATED:
                            ...
                            userMeta =
                                    JsonUtils.fromJson(event.getData().getData(), UserMeta.class);
                            if (userMeta != null) {
    ...
    public List<UserMeta> loadUser(UserMeta userMeta) throws MetaStorageException {
        ...
        try {
            mutex.acquire();
            if (this.client.checkExists().forPath(USER_NODE_PREFIX)
                    == null) { // 节点不存在，说明系统中第一个用户还没有被创建
                this.client
                        .create()
                        .creatingParentsIfNeeded()
                        .withMode(CreateMode.PERSISTENT)
                        .forPath(
                                USER_NODE_PREFIX + "/" + userMeta.getUsername(),
                                JsonUtils.toJson(userMeta));
            }
            ...
            for (String username : usernames) {
                ...
                if (user == null) {
                    logger.error(
                            "resolve data from " + USER_NODE_PREFIX + "/" + username + " error");
                    continue;
                }
                users.add(user);
            }
    ...
    public void updateTimeseriesData(Map<String, Double> timeseriesData, long iginxid, long version)
        ...
        try {
            ...
            this.client
                    .setData()
                    .forPath(
                            POLICY_NODE_PREFIX + "/" + iginxid, String.valueOf(version).getBytes());
        } catch (Exception e) {
    ...
    public Map<String, Double> getTimeseriesData() {
        ...
        try {
            ...
            for (String child : children) {
                ...
                if (tmp == null) {
                    logger.error(
                            "resolve data from " + TIMESERIES_NODE_PREFIX + "/" + child + " error");
                    continue;
                }
                tmp.forEach(
                        (timeseries, value) -> {
                        ...
                        switch (event.getType()) {
                            ...
                            case NODE_UPDATED:
                                ...
                                if (pathParts.length == 2 && isNumeric(pathParts[1])) {
                                    version =
                                            Integer.parseInt(new String(event.getData().getData()));
    ...
    public int updateVersion() {
        ...
        try {
            version =
                    Integer.parseInt(
                            new String(client.getData().forPath(POLICY_VERSION)).split("\t")[0]);
        ...
        try {
            ...
            if (this.client.checkExists().forPath(TRANSFORM_NODE_PREFIX) == null) {
                ...
                for (String className : classNames) {
                    byte[] data =
                            this.client.getData().forPath(TRANSFORM_NODE_PREFIX + "/" + className);
                ...
                (curatorFramework, event) -> {
                    ...
                    switch (event.getType()) {
                        ...
                        case NODE_UPDATED:
                            ...
                            taskMeta =
                                    JsonUtils.fromJson(
                                            event.getData().getData(), TransformTaskMeta.class);
                            if (taskMeta != null) {
    ...
    public Map<String, Long> loadTimeseriesHeat() throws Exception {
        ...
        for (String child : children) {
            byte[] data =
                    this.client.getData().forPath(STATISTICS_TIMESERIES_HEAT_PREFIX + "/" + child);
            long heat = JsonUtils.fromJson(data, Long.class);
        ...
        try {
            if (this.client.checkExists().forPath(STATISTICS_TIMESERIES_HEAT_PREFIX) != null) {
                this.client
                        ...
                        .forPath(STATISTICS_TIMESERIES_HEAT_PREFIX);
            }
        } catch (Exception e) {
    ...
    public void incrementTimeseriesHeatCounter() throws MetaStorageException {
        try {
            if (this.client.checkExists().forPath(STATISTICS_TIMESERIES_HEAT_COUNTER_PREFIX)
                    == null) {
                this.client
                        .create()
                        .creatingParentsIfNeeded()
                ...
                int counter =
                        JsonUtils.fromJson(
                                this.client
                                        .getData()
                                        .forPath(STATISTICS_TIMESERIES_HEAT_COUNTER_PREFIX),
                                Integer.class);
                this.client
                        .setData()
                        .forPath(
                                STATISTICS_TIMESERIES_HEAT_COUNTER_PREFIX,
                                JsonUtils.toJson(counter + 1));
            }
        } catch (Exception e) {
            throw new MetaStorageException(
    ...
    public void releaseTimeseriesHeatCounter() throws MetaStorageException {
        try {
            ...
            throw new MetaStorageException(
                    "encounter error when releasing timeseries heat mutex: ", e);
        } finally {
            timeseriesHeatCounterMutexLock.unlock();
        }
    }
        ...
        for (Entry<FragmentMeta, Long> writeRequestsEntry : writeRequestsMap.entrySet()) {
            if (writeRequestsEntry.getValue() > 0) {
                ...
                this.client
                        ...
                        .forPath(
                                requestsPath,
                                JsonUtils.toJson(requests + writeRequestsEntry.getValue()));
                ...
                this.client
                        ...
                        .forPath(
                                pointsPath,
                                JsonUtils.toJson(points + writeRequestsEntry.getValue()));
            }
        }
    ...
    public void removeFragmentRequests() throws MetaStorageException {
        try {
            if (this.client.checkExists().forPath(STATISTICS_FRAGMENT_REQUESTS_PREFIX_WRITE)
                    != null) {
                this.client
                        .delete()
                        .deletingChildrenIfNeeded()
            ...
            if (this.client.checkExists().forPath(STATISTICS_FRAGMENT_REQUESTS_PREFIX_READ)
                    != null) {
                this.client
                        .delete()
                        .deletingChildrenIfNeeded()
    ...
    public void incrementFragmentRequestsCounter() throws MetaStorageException {
        try {
            if (this.client.checkExists().forPath(STATISTICS_FRAGMENT_REQUESTS_COUNTER_PREFIX)
                    == null) {
                this.client
                        .create()
                        .creatingParentsIfNeeded()
    ...
    public void incrementFragmentHeatCounter() throws MetaStorageException {
        try {
            if (this.client.checkExists().forPath(STATISTICS_FRAGMENT_HEAT_COUNTER_PREFIX)
                    == null) {
                this.client
                        .create()
                        .creatingParentsIfNeeded()
                ...
                int counter =
                        JsonUtils.fromJson(
                                this.client
                                        .getData()
                                        .forPath(STATISTICS_FRAGMENT_HEAT_COUNTER_PREFIX),
                                Integer.class);
                this.client
                        .setData()
                        .forPath(
            ...
            throw new MetaStorageException(
                    "encounter error when updating fragment heat counter: ", e);
        }
    }
    ...
    public void resetFragmentHeatCounter() throws MetaStorageException {
        try {
            ...
            throw new MetaStorageException(
                    "encounter error when resetting fragment heat counter: ", e);
        }
    }
    ...
    public void releaseFragmentHeatCounter() throws MetaStorageException {
        try {
            ...
            throw new MetaStorageException(
                    "encounter error when releasing latency counter mutex: ", e);
        } finally {
            fragmentHeatCounterMutexLock.unlock();
        }
    }
    ...
    public boolean proposeToReshard() throws MetaStorageException {
        try {
            ...
            if (this.client.checkExists().forPath(RESHARD_STATUS_NODE_PREFIX) == null) {
                ...
                status =
                        JsonUtils.fromJson(
                                this.client.getData().forPath(RESHARD_STATUS_NODE_PREFIX),
                                ReshardStatus.class);
    ...
    public void lockReshardStatus() throws MetaStorageException {
        try {
            ...
            throw new MetaStorageException(
                    "encounter error when acquiring reshard status mutex: ", e);
        }
    }
    ...
    public void releaseReshardStatus() throws MetaStorageException {
        try {
            ...
            throw new MetaStorageException(
                    "encounter error when releasing reshard status mutex: ", e);
        } finally {
            reshardStatusMutexLock.unlock();
        }
    }
    ...
    public void lockReshardCounter() throws MetaStorageException {
        try {
            ...
            throw new MetaStorageException(
                    "encounter error when acquiring reshard counter mutex: ", e);
        }
    }
    ...
    public void incrementReshardCounter() throws MetaStorageException {
        try {
            if (this.client.checkExists().forPath(RESHARD_COUNTER_NODE_PREFIX) == null) {
                ...
                int counter =
                        JsonUtils.fromJson(
                                this.client.getData().forPath(RESHARD_COUNTER_NODE_PREFIX),
                                Integer.class);
                this.client
                        .setData()
    ...
    public void releaseReshardCounter() throws MetaStorageException {
        try {
            ...
            throw new MetaStorageException(
                    "encounter error when releasing reshard counter mutex: ", e);
        } finally {
            reshardCounterMutexLock.unlock();
        }
    }
    ...
    public long getMaxActiveEndTimeStatistics() throws MetaStorageException {
        try {
            if (this.client.checkExists().forPath(MAX_ACTIVE_END_TIME_STATISTICS_NODE) != null) {
                return JsonUtils.fromJson(
                        this.client.getData().forPath(MAX_ACTIVE_END_TIME_STATISTICS_NODE),
                        Long.class);
            }
        } catch (Exception e) {
            throw new MetaStorageException(
