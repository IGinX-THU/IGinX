...
public class MetricsResource {
        ...
        try {
            ...
            return setHeaders(
                            Response.status(Status.BAD_REQUEST)
                                    .entity("Error occurred during execution\n"))
                    .build();
        }
    }
    ...
    private QueryResult annoQuery(Query query, QueryParser parser, String jsonStr)
            throws Exception {
        // 查找出所有符合tagkv的序列路径
    ...
    public QueryResult getAnnoDataQueryFromTitle(Query query, QueryResult result) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            for (int j = 0; j < data.getPaths().size(); j++) {
                if (title.equals(".*")
                        || title.isEmpty()
                        || (data.getTitles().get(j) != null
                                && data.getTitles().get(j).equals(title))) {
            ...
            for (int j = 0; j < result.getQueryResultDatasets().size(); j++) {
                ...
                paths =
                        parser.getPrefixPaths(
                                query.getQueryMetrics().get(i).getAnnotationLimit().getTag(),
                                data.getPaths());
            }
