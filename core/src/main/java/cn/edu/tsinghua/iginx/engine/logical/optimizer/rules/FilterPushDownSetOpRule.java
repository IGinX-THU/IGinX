...
public class FilterPushDownSetOpRule extends Rule{
    ...
    public boolean matches(RuleCall call) {
        ...
        AbstractBinaryOperator operator = (AbstractBinaryOperator) ((OperatorSource)select.getSource()).getOperator();
    ...
    public void onMatch(RuleCall call) {
        Select select = (Select) call.getMatchedRoot();
        AbstractBinaryOperator setOp = (AbstractBinaryOperator) ((OperatorSource)select.getSource()).getOperator();
        Pair<List<String>, List<String>> order = getSetOpOrder(setOp);

        if (order.k.size() == order.v.size() &&
                IntStream.range(0, order.k.size()).allMatch(i -> order.k.get(i).equals(order.v.get(i)))){
            // 如果左右输入的Order相同，那么直接将Filter下推到SetOp的两侧
            Select leftSelect = new Select(setOp.getSourceA(), select.getFilter().copy(), select.getTagFilter().copy());
            Select rightSelect = new Select(setOp.getSourceB(), select.getFilter().copy(), select.getTagFilter().copy());
            ...
            Select leftSelect = new Select(setOp.getSourceA(), select.getFilter().copy(), select.getTagFilter().copy());
            Select rightSelect = new Select(setOp.getSourceB(), replaceFilterPath(select.getFilter().copy(), pathMap),
    ...
    private Pair<List<String>, List<String>> getSetOpOrder(AbstractBinaryOperator operator){
        if(operator instanceof Union){
            ...
            return new Pair<>(((Intersect) operator).getLeftOrder(), ((Intersect) operator).getRightOrder());
    ...
    private Filter replaceFilterPath(Filter filter, Map<String, String> pathMap){
        filter.accept(new FilterVisitor() {
            ...
            public void visit(PathFilter filter) {
                filter.setPathA(pathMap.getOrDefault(pathMap.get(filter.getPathA()), filter.getPathA()));
                filter.setPathB(pathMap.getOrDefault(pathMap.get(filter.getPathB()), filter.getPathB()));
            }
