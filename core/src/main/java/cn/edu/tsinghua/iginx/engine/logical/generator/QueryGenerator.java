...
public class QueryGenerator extends AbstractGenerator {
  ...
  protected Operator generateRoot(Statement statement) {
    ...
    if (selectStatement.getQueryType() == QueryType.GroupByQuery) {
                  ...
                  v.forEach(
                      expression -> {
                        ...
                        if (k.equals("")) {
                          ...
                          logger.info(
                              "function: " + k + ", wrapped path: " + expression.getPathName());
      ...
      selectStatement
          .getBaseExpressionMap()
          .forEach((k, v) -> v.forEach(expression -> selectedPath.add(expression.getPathName())));
      queryList.add(
              ...
              if (fromPart.getType() == FromPartType.SubQueryFromPart) {
                ...
                Pair<Map<TimeInterval, List<FragmentMeta>>, List<FragmentMeta>> pair =
                    getFragmentsByTSInterval(
                        selectStatement, new TimeSeriesInterval(prefix, prefix));
                Map<TimeInterval, List<FragmentMeta>> fragments = pair.k;
                List<FragmentMeta> dummyFragments = pair.v;
                joinList.add(
                    mergeRawData(
                  ...
                  new AddSchemaPrefix(
                      new OperatorSource(
                          new Project(
                              ...
                              pathMatchPrefix(
                                  pathList, meta.getTsInterval().getTimeSeries(), schemaPrefix),
                              tagFilter)),
                      schemaPrefix));
            }
          });
  ...
  private Pair<Map<TimeInterval, List<FragmentMeta>>, List<FragmentMeta>> getFragmentsByTSInterval(
      SelectStatement selectStatement, TimeSeriesInterval interval) {
