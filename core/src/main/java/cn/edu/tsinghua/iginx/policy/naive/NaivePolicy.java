...
public class NaivePolicy implements IPolicy {
  ...
  private Pair<List<FragmentMeta>, List<StorageUnitMeta>>
    ...
    if (paths == null || paths.isEmpty()) {
      ...
      pair =
          generateFragmentAndStorageUnitByTimeSeriesIntervalAndTimeInterval(
              null, null, timeInterval.getStartTime(), Long.MAX_VALUE, storageEngineIdList);
      fragmentList.add(pair.k);
      storageUnitList.add(pair.v);
    ...
    pair =
        generateFragmentAndStorageUnitByTimeSeriesIntervalAndTimeInterval(
            null, paths.get(0), timeInterval.getStartTime(), Long.MAX_VALUE, storageEngineIdList);
    fragmentList.add(pair.k);
    storageUnitList.add(pair.v);
  ...
  private Pair<Map<TimeSeriesRange, List<FragmentMeta>>, List<StorageUnitMeta>>
    ...
    for (int i = 0; i < clients.length * instancesNumPerClient - 1; i++) {
      ...
      //            storageUnit = new StorageUnitMeta(masterId, getStorageEngineList().get(i * 2 %
      // getStorageEngineList().size()).getId(), masterId, true);
      for (int j = i + 1; j < i + replicaNum; j++) {
        ...
        // StorageUnitMeta(RandomStringUtils.randomAlphanumeric(16), getStorageEngineList().get((i *
        // 2 + 1) % getStorageEngineList().size()).getId(), masterId, false));
      }
