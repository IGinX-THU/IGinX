...
public class NestedLoopOuterJoinLazyStream extends BinaryLazyStream {
    ...
    public NestedLoopOuterJoinLazyStream(
                ...
                if (!matchedStreamBRowIndexSet.contains(i)) {
                    Row unmatchedRow =
                            RowUtils.constructUnmatchedRow(
                                    header, streamBCache.get(i), anotherRowSize, false);
                    lastPart.add(unmatchedRow);
                }
            }
        }
            ...
            if (outerJoin.getOuterJoinType() == OuterJoinType.RIGHT) {
                row =
                        RowUtils.constructNewRow(
                                header, nextA, nextB, indexOfJoinColumnsInTable, false);
            } else {
                row =
                        RowUtils.constructNewRow(
                                header, nextA, nextB, indexOfJoinColumnsInTable, true);
            }
            nextB = null;
            return row;
        }
