          ...
          .forEach(
              e -> {
                ...
                Map<String, Integer> schemaMapping =
                    JsonUtils.transform(e.getValue().toString(StandardCharsets.UTF_8));
    ...
    try {
      ...
      GetResponse response =
          this.client
              .getKVClient()
              .get(
                  ByteSequence.from((FRAGMENT_PREFIX + tsInterval.toString()).getBytes()),
                  GetOption.newBuilder()
                      .withPrefix(
                          ByteSequence.from((FRAGMENT_PREFIX + tsInterval.toString()).getBytes()))
                      .build())
      ...
      if (response.getKvs().isEmpty()) {
        client
            .getKVClient()
            .delete(ByteSequence.from((FRAGMENT_PREFIX + tsInterval.toString()).getBytes()));
      }
      client
          .getKVClient()
          .put(
