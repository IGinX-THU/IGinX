...
public class GreedyStorageMigrationPolicy extends StorageMigrationPolicy {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(GreedyStorageMigrationPolicy.class);
    ...
    @Override
    public StorageMigrationPlan generateMigrationPlans(
            long sourceStorageId, boolean migrationData) {
        logger.info(
        ...
        for (StorageUnitMeta meta : metaManager.getStorageUnits()) {
            logger.info(
                    "[FaultTolerance][MigrationPolicy][iginx={}, id={}] current storage unit info: {}",
                    metaManager.getIginxId(),
                    sourceStorageId,
                    meta.toString());
        }
        // discard、dummy 的分片不需要迁移，只迁移很正常状态的分片
        Map<Long, List<StorageUnitMeta>> storageUnitsMap =
                metaManager
                        .getStorageUnits()
                        .stream()
                        .filter(e -> !e.isDummy())
        ...
        for (StorageUnitMeta meta : storageUnits) {
            logger.info(
                    "[FaultTolerance][MigrationPolicy][iginx={}, id={}] du={} need to migration.",
                    metaManager.getIginxId(),
                    sourceStorageId,
                    meta.getId());
        }
        ...
        for (StorageUnitMeta storageUnit : storageUnits) {
            ...
            if (!storageUnit.isMaster()) {
                ...
                for (StorageUnitMeta unit : metaManager.getStorageUnits()) {
                    ...
                    if (!unit.isMaster()
                            && Objects.equals(unit.getMasterId(), storageUnit.getId())) {
            ...
            logger.info(
                    "[FaultTolerance][MigrationPolicy][iginx={}, id={}] du={} avoid migration to {}, decide migration to {}",
                    metaManager.getIginxId(),
                    sourceStorageId,
                    storageUnit.getId(),
                    avoid,
        ...
        return new StorageMigrationPlan(
                sourceStorageId, migrationData, migrationMap, metaManager.getIginxId());
    }
}
