...
public class SimulationBasedMigrationPolicy extends MigrationPolicy {
    ...
    public void migrate(
        ...
        List<List<Queue<MigrationTask>>> candidateMigrationTaskQueueList =
                createParallelQueueWithoutPriority(migrationTasks);
        ...
        for (int currIndex = 0; currIndex < candidateMigrationTaskQueueList.size(); currIndex++) {
            List<Queue<MigrationTask>> migrationTaskQueueList =
                    candidateMigrationTaskQueueList.get(currIndex);
            // 模拟正在进行的迁移任务, 任务->起始时间
            ...
            while (!isAllQueueEmpty(migrationTaskQueueList)) {
                simulateOneRoundMigration(
                        migrationTaskQueueList, nodeLoadMap, workingMigrationTask, currTime);
                // 算出一下个被执行完的任务，并更新时间以及不均衡度
                MigrationTask nextFinishedMigrationTask =
                        getNextFinishedMigrationTask(workingMigrationTask);
                long finishedTime =
                        nextFinishedMigrationTask.getMigrationSize()
                                + workingMigrationTask.get(nextFinishedMigrationTask);
        ...
        List<Queue<MigrationTask>> migrationTaskQueueList =
                candidateMigrationTaskQueueList.get(targetIndex);
    ...
    private List<List<Queue<MigrationTask>>> createParallelQueueWithoutPriority(
        ...
        for (Entry<String, List<MigrationTask>> edgeMigrationTasksEntry :
            ...
            // 获取该列表的所有组合形式
            List<List<MigrationTask>> allPossibleMigrationTaskList =
                    MigrationUtils.combination(edgeMigrationTasks, edgeMigrationTasks.size());
        ...
        while (!Arrays.equals(listEndStatus, listCurrIndex)) {
            ...
            for (int i = 0; i < listCurrIndex.length; i++) {
                Queue<MigrationTask> migrationTaskQueue =
                        new LinkedBlockingQueue<>(migrationTaskLists.get(i).get(listCurrIndex[i]));
