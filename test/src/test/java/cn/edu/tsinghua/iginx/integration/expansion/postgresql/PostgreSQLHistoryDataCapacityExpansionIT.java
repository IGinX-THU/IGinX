...
public class PostgreSQLHistoryDataCapacityExpansionIT extends CapacityExpansionIT {
    ...
    public void testSchemaPrefix() throws Exception {
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 5433, \""
                        + ENGINE_TYPE
                        + "\", \"username:postgres, password:postgres, has_data:true, schema_prefix:expansion, is_read_only:true\");");
    ...
    public void testDataPrefix() throws Exception {
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 5433, \""
                        + ENGINE_TYPE
                        + "\", \"username:postgres, password:postgres, has_data:true, data_prefix:test, is_read_only:true\");");
    ...
    public void testAddSameDataPrefixWithDiffSchemaPrefix_AND_testRemoveHistoryDataSource()
        ...
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 5433, \""
                        + ENGINE_TYPE
                        + "\", \"username:postgres, password:postgres, has_data:true, data_prefix:test, schema_prefix:p1, is_read_only:true\");");
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 5433, \""
                        + ENGINE_TYPE
                        + "\", \"username:postgres, password:postgres, has_data:true, data_prefix:test, schema_prefix:p2, is_read_only:true\");");
        ...
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 5433, \""
                        + ENGINE_TYPE
                        + "\", \"username:postgres, password:postgres, has_data:true, data_prefix:test, is_read_only:false\");");
        statement = "select * from test";
        expect =
                "ResultSets:\n"
                        + "+---+---------------------+--------------------------+\n"
        ...
        removedStorageEngineList.set(
                0, new RemovedStorageEngineInfo("127.0.0.1", 5433, "p2", "test"));
