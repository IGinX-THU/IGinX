...
public class TransformIT {
  ...
  private void registerTask(String task, String className, String filename)
    ...
    session.executeSql(
        String.format(
            CREATE_SQL_FORMATTER, task, className, SqlPathUtil.escapePathForSql(filename)));
  }
  ...
  @Test
  public void noOutputPrefixTest() {
    // yaml is illegal
    LOGGER.info("noOutputPrefixTest");
    try {
      String yamlFileName =
  ...
  @Test
  public void OutputPrefixInSQLTest() {
    // yaml has redundant params(ignored)
    LOGGER.info("OutputPrefixInSQLTest");
    String[] taskList = {"RowSumTransformer", "AddOneTransformer", "SumTransformerNoKey"};
    testMixJobs(
        taskList,
        "TransformMixedPythonAndSQLWrongOutputPrefix.yaml",
        "export_file_mixed_python_And_SQL_OutputPrefixInSQL.txt");
  }
                      ...
                      sql -> {
                        ...
                        while (matcher.find()) {
                          ...
                          if (!filePath.isAbsolute()) {
                            String absolutePath =
                                SqlPathUtil.escapePathForSql(filePath.getAbsolutePath());
