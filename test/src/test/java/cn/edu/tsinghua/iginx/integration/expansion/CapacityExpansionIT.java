...
public class CapacityExpansionIT implements BaseCapacityExpansionIT {
    ...
    private void addStorageEngine(boolean hasData) throws SessionException, ExecutionException {
        if (ENGINE_TYPE.toLowerCase().contains("iotdb"))
            ...
            session.executeSql("ADD STORAGEENGINE (\"127.0.0.1\", 8087, \"" + ENGINE_TYPE + "\", \"url:http://localhost:8087/, username:user, password:12345678, sessionPoolSize:20, has_data:" + hasData + ", is_read_only:true, token:testToken, organization:testOrg\");");
