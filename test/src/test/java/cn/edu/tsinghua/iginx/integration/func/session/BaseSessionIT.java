...
public abstract class BaseSessionIT extends BaseSessionConcurrencyIT {
  ...
  public void clearData() throws ExecutionException, SessionException {
    ...
    if (res != null && res.getParseErrorMsg() != null && !res.getParseErrorMsg().equals("")) {
      logger.error(
          "Statement: \"{}\" execute fail. Caused by: {}.", clearData, res.getParseErrorMsg());
      fail();
    }
  }
  ...
  public void sessionTest() throws ExecutionException, SessionException, InterruptedException {
    ...
    if (isAbleToDelete) {
      ...
      SessionQueryDataSet delDsAvgDataSet =
          session.downsampleQuery(paths, START_TIME, END_TIME + 1, AggregateType.AVG, PRECISION);
      ...
      SessionAggregateQueryDataSet delDataAvgSet =
          session.aggregateQuery(delDataInColumnPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      SessionQueryDataSet dsDelDataInColSet =
          session.downsampleQuery(
              delDataInColumnPaths, START_TIME, END_TIME + 1, AggregateType.AVG, PRECISION);
    ...
    for (int i = 0; i < dtQueryLen; i++) {
      ...
      for (int j = 0; j < 6; j++) {
        ...
        switch (currPathPos) {
          ...
          case 4:
            assertEquals(
                ((END_TIME - i) + 4 + 0.01) * 999, changeResultToDouble(result.get(j)), delta);
            break;
          case 5:
    ...
    for (int i = 0; i < 4; i++) {
      ...
      switch (currPathPos) {
        ...
        case 3:
          assertEquals(
              (float) (END_TIME + 3 + 0.01), changeResultToFloat(dtMaxResult[i]), (float) delta);
          break;
        case 4:
    ...
    for (int i = 0; i < 4; i++) {
      ...
      switch (currPathPos) {
        case 1:
          assertEquals(
              (START_TIME + END_TIME) / 2.0 + 1, changeResultToDouble(dtAvgResult[i]), delta);
          break;
        case 2:
    ...
    if (isAbleToDelete) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        switch (currPathPos) {
          case 1:
            assertEquals(
                ((START_TIME + END_TIME) * TIME_PERIOD / 2.0
                            - (END_TIME
                                    - (dtDelStartTime - START_TIME)
                                    + END_TIME
                                    - (dtDelEndTime - 1 - START_TIME))
                                * dtDelTimePeriod
                                / 2.0)
          ...
          case 3:
            assertEquals(
                ((START_TIME + END_TIME) * TIME_PERIOD / 2.0
                            - (dtDelStartTime + dtDelEndTime - 1) * dtDelTimePeriod / 2.0)
                        / (TIME_PERIOD - dtDelTimePeriod)
                    + 3.01,
                changeResultToDouble(dtDelPartAvgResult[i]),
                delta * 10000);
        ...
        for (int j = 0; j < 6; j++) {
          ...
          if (currPathPos < dtDelColumnNum) {
            ...
            switch (getPathNum(dtDelColResPaths.get(j)) - currPath) {
              ...
              case 1:
                assertEquals(
                    (int) ((END_TIME - timestamp) + 1), changeResultToInteger(result.get(j)));
                break;
              case 2:
                assertEquals((timestamp + 2) * 1000, result.get(j));
                break;
      ...
      SessionAggregateQueryDataSet dtDelColAvgDataSet =
          session.aggregateQuery(dTDeleteAggrPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
    ...
    if (ifNeedCapExp) {
      ...
      SessionAggregateQueryDataSet addStCountDataSet =
          session.aggregateQuery(addStoragePaths, START_TIME, END_TIME + 1, AggregateType.COUNT);
      ...
      SessionAggregateQueryDataSet addStAvgDataSet =
          session.aggregateQuery(addStoragePaths, START_TIME, END_TIME + 1, AggregateType.AVG);
