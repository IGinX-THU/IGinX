...
public class IoTDB12HistoryDataCapacityExpansionIT extends CapacityExpansionIT {
    ...
    public void testSchemaPrefix() throws Exception {
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 6668, \""
                        + ENGINE_TYPE
                        + "\", \"username:root, password:root, sessionPoolSize:20, has_data:true, schema_prefix:expansion, is_read_only:true\");");
    ...
    public void testDataPrefix() throws Exception {
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 6668, \""
                        + ENGINE_TYPE
                        + "\", \"username:root, password:root, sessionPoolSize:20, has_data:true, data_prefix:test, is_read_only:true\");");
    ...
    @Test
    public void testAddSameDataPrefixWithDiffSchemaPrefix_AND_testRemoveHistoryDataSource()
            throws Exception {
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 6668, \""
                        + ENGINE_TYPE
                        + "\", \"username:root, password:root, sessionPoolSize:20, has_data:true, data_prefix:test, schema_prefix:p1, is_read_only:true\");");
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 6668, \""
                        + ENGINE_TYPE
                        + "\", \"username:root, password:root, sessionPoolSize:20, has_data:true, data_prefix:test, schema_prefix:p2, is_read_only:true\");");
        ...
        session.executeSql(
                "ADD STORAGEENGINE (\"127.0.0.1\", 6668, \""
                        + ENGINE_TYPE
                        + "\", \"username:root, password:root, sessionPoolSize:20, has_data:true, data_prefix:test, is_read_only:false\");");
        statement = "select * from test";
        expect =
                "ResultSets:\n"
                        + "+---+---------------------+--------------------------+\n"
        ...
        removedStorageEngineList.set(
                0, new RemovedStorageEngineInfo("127.0.0.1", 6668, "p2", "test"));
