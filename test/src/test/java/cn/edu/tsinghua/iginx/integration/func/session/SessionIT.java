    ...
    public void sessionTest() throws ExecutionException, SessionException, InterruptedException {
        ...
        if (isAbleToDelete) {
            ...
            SessionQueryDataSet delDsAvgDataSet =
                    session.downsampleQuery(
                            paths, START_KEY, END_KEY + 1, AggregateType.AVG, PRECISION);
            ...
            SessionAggregateQueryDataSet delDataAvgSet =
                    session.aggregateQuery(
                            delDataInColumnPaths, START_KEY, END_KEY + 1, AggregateType.AVG);
        ...
        for (int i = 0; i < dtQueryLen; i++) {
            ...
            for (int j = 0; j < 6; j++) {
                ...
                switch (currPathPos) {
                    case 1:
                        assertEquals(
                                (int) ((END_KEY - i) + 1), changeResultToInteger(result.get(j)));
                        break;
                    case 2:
        ...
        if (isAbleToDelete) {
            ...
            SessionAggregateQueryDataSet dtDelColAvgDataSet =
                    session.aggregateQuery(
                            dTDeleteAggrPaths, START_KEY, END_KEY + 1, AggregateType.AVG);
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (currPathPos < dtDelColumnNum) {
                    ...
                    switch (currPathPos) {
                        ...
                        case 2:
                            assertEquals(
                                    (START_KEY + END_KEY) * 500.0 + 2000, dtDelColAvgResult[i]);
                            break;
                        case 3:
                            assertEquals(
                                    (START_KEY + END_KEY) / 2.0 + 3 + 0.01,
        ...
        if (ifNeedCapExp) {
            ...
            SessionAggregateQueryDataSet addStCountDataSet =
                    session.aggregateQuery(
                            addStoragePaths, START_KEY, END_KEY + 1, AggregateType.COUNT);
            ...
            SessionAggregateQueryDataSet addStAvgDataSet =
                    session.aggregateQuery(
                            addStoragePaths, START_KEY, END_KEY + 1, AggregateType.AVG);
