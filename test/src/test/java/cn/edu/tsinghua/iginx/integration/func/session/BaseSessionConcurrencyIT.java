  ...
  public void multiThreadTestBad()
    ...
    for (int i = 0; i < mulStQueryLen; i++) {
      mulStInsertTasks[i] =
          new BaseSessionConcurrencyIT.MultiThreadTask(
              1, getPaths(currPath + i, 1), START_TIME, END_TIME, TIME_PERIOD, 1, null, 6888);
    ...
    if (isAbleToDelete) {
      ...
      SessionAggregateQueryDataSet delPSAvgDataSet =
          session.aggregateQuery(mulDelPSPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      SessionAggregateQueryDataSet delPTAvgDataSet =
          session.aggregateQuery(mulDelPTPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      for (int i = 0; i < delASThreadNum; i++) {
        delASTasks[i] =
            new BaseSessionConcurrencyIT.MultiThreadTask(
                2, getPaths(currPath + i, 1), START_TIME, END_TIME + 1, TIME_PERIOD, 1, null, 6888);
      ...
      SessionAggregateQueryDataSet delASAvgDataSet =
          session.aggregateQuery(mulDelASPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
      ...
      SessionAggregateQueryDataSet delATAvgDataSet =
          session.aggregateQuery(mulDelATPaths, START_TIME, END_TIME + 1, AggregateType.AVG);
