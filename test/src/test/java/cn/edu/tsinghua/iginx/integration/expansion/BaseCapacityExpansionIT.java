...
public abstract class BaseCapacityExpansionIT {
  ...
  private void testAddAndRemoveStorageEngineWithPrefix() {
    ...
    try {
      SessionExecuteSqlResult explain = session.executeSql("explain physical select wt01.status2 from nt.wf03;");
      logger.info("explain physical result: {}", explain.getResultInString(false,""));
    } catch (SessionException | ExecutionException e) {
      throw new RuntimeException(e);
    }
