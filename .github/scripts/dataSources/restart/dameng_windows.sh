...
if wsl docker ps -a | grep -q "dm8-$port"; then
  ...
  wsl docker run -d -p $port:5236 --restart=always \
    --name=dm8-$port \
    --privileged=true \
...
# 检查容器是否运行
if ! wsl docker ps | grep -q "dm8-$port"; then
  log "ERROR: Container is not running after startup/restart"
  wsl docker logs "dm8-$port" || true
  exit 1
fi
...
while [ $elapsed -lt $total_wait ]; do
  # 检查最近的日志（使用 WSL Docker）
  recent_logs=$(wsl docker logs --since $restart_time "dm8-$port" 2>&1)
  ...
  log "'DM Database is OK' not found in the logs, retrying in 10 seconds..."
