...
sh -c "sudo sed -i '' 's/^# enable_mem_control=.*$/enable_mem_control=false/' apache-iotdb-0.12.6-server-bin/conf/iotdb-engine.properties"

sh -c "sudo sed -i '' 's/^# enable_wal=.*$/enable_wal=false/' apache-iotdb-0.12.6-server-bin/conf/iotdb-engine.properties"

sh -c "sudo sed -i '' 's/^# enable_timed_close_tsfile=.*$/enable_timed_close_tsfile=false/' apache-iotdb-0.12.6-server-bin/conf/iotdb-engine.properties"
