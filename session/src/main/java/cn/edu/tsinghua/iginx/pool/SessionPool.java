...
public class SessionPool {
  ...
  public SessionQueryDataSet downsampleQuery(
      List<String> paths, long startTime, long endTime, AggregateType aggregateType, long precision)
      throws SessionException, ExecutionException {
    SessionQueryDataSet sessionQueryDataSet = null;
    for (int i = 0; i < RETRY; i++) {
      Session session = getSession();
      try {
        sessionQueryDataSet =
            session.downsampleQuery(paths, startTime, endTime, aggregateType, precision);
        putBack(session);
        return sessionQueryDataSet;
      } catch (SessionException e) {
        // TException means the connection is broken, remove it and get a new one.
  ...
  public SessionQueryDataSet downsampleQuery(
    ...
    for (int i = 0; i < RETRY; i++) {
      ...
      try {
        sessionQueryDataSet =
            session.downsampleQuery(paths, startTime, endTime, aggregateType, precision, tagsList);
        putBack(session);
        return sessionQueryDataSet;
      } catch (SessionException e) {
        // TException means the connection is broken, remove it and get a new one.
  ...
  public CurveMatchResult curveMatch(
      List<String> paths, long startTime, long endTime, List<Double> curveQuery, long curveUnit)
      throws SessionException, ExecutionException {
