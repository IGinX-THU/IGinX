...
public class ResultFormatTest {
    ...
    public void SessionExecuteSqlResultFormatTest() {
        ...
        SessionExecuteSqlResult result = new SessionExecuteSqlResult(sqlType, keys, paths, data,
            types);

        List<String> timePrecisionList = Arrays.asList("ns", "us", "ms", "s");
        for (String timePrecision : timePrecisionList) {
            List<List<String>> cacheResult = result
                .getResultInList(true, FormatUtils.DEFAULT_TIME_FORMAT, timePrecision);
            ...
            for (int i = 0; i < keys.length; i++) {
                long timestamp = TimeUtils
                    .getTimeInNs(keys[i], TimeUtils.strToTimePrecision(timePrecision));
