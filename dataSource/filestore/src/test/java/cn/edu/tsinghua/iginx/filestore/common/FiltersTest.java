...
class FiltersTest {
  ...
  @Test
  void testSuperSet() {
    AndFilter root = new AndFilter(
        Arrays.asList(
            new OrFilter(
                Arrays.asList(
                    new KeyFilter(Op.GE, 1),
                    new KeyFilter(Op.LE, 3)
                )
            ),
            new OrFilter(
                Arrays.asList(
    ...
    Filter superSet = Filters.superSet(root, Filters.removeNonKeyFilter());
    Filter expected = new OrFilter(
        Arrays.asList(
            new KeyFilter(Op.GE, 1),
            new KeyFilter(Op.LE, 3)
        )
