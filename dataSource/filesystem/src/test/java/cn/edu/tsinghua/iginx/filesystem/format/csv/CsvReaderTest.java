...
class CsvReaderTest {
  ...
  void testTpchLineitemTsvWithTypeHint() throws IOException, PhysicalException {
    ...
    try(InputStream is = getClass().getClassLoader().getResourceAsStream("data/tpch/lineitem_hint.csv")) {
      if(is == null) {
        throw new IOException("Resource not found");
      }
      Files.copy(is, path, StandardCopyOption.REPLACE_EXISTING);
    ...
    Header expectedHeader = new Header(Field.KEY, expectedField);
    List<Row> expectedRows = Arrays.asList(
        new Row(expectedHeader,0L,new Object[]{1,156,4,1,17.0,17954.55,0.04,0.02,"N","O",826646400000L,824054400000L,827424000000L,"DELIVER IN PERSON","TRUCK","egular courts above the"}),
        new Row(expectedHeader,1L,new Object[]{1,68,9,2,36.0,34850.16,0.09,0.06,"N","O",829238400000L, 825436800000L, 829929600000L,"TAKE BACK RETURN","MAIL","ly final dependencies: slyly bold "}),
        new Row(expectedHeader,2L,new Object[]{1,64,5,3,8.0,7712.48,0.10,0.02,"N","O",822844800000L,825955200000L,823017600000L,"TAKE BACK RETURN","REG AIR","riously. regular, express dep"})
    );
