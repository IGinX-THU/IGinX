...
class CsvReaderTest {
  ...
  void testTpchLineitemTsvWithTypeHint() throws IOException, PhysicalException {
    Path path = Files.createTempFile("testTpchLineitemWithTypeHint", ".csv");
    try (InputStream is =
  ...
  void testCustomDatetimeFormat() throws IOException, PhysicalException {
    Path path = Files.createTempFile("testTpchLineitemWithCustomCustomDatetime", ".csv");
    try (InputStream is =
  ...
  void testAllowDuplicateColumnNamesFalse() throws IOException, PhysicalException {
    Path path = Files.createTempFile("testTpchLineitemWithDuplicateColumns", ".csv");
    try (InputStream is =
        getClass()
            .getClassLoader()
  ...
  @Test
  void testCustomSampleSize() throws IOException, PhysicalException {
    Path path = Files.createTempFile("testTpchLineitemWithCustomSampleSize", ".csv");
    try (InputStream is =
        getClass()
            .getClassLoader()
            .getResourceAsStream("data/tpch/lineitem_customsamplesize.csv")) {
      if (is == null) {
        throw new IOException("Resource not found");
      }
      Files.copy(is, path, StandardCopyOption.REPLACE_EXISTING);
