...
public class PredicateDeduper {
  ...
  public static Predicate simplify(@Nullable Predicate predicate) {
    ...
    if (andParts.size() > 1) {
      // AND 语境：丢弃更弱的（被别人包含的）
      List<Predicate> simplified =
          andParts.stream()
              .map(PredicateDeduper::simplify)
              .filter(Objects::nonNull)
              .distinct()
              .collect(Collectors.toList());
      Preconditions.checkState(
          !simplified.isEmpty(), "After simplification, AND parts should not be empty");
    ...
    if (orParts.size() > 1) {
      ...
      List<Predicate> simplified =
          orParts.stream()
              ...
              .collect(Collectors.toList());
      Preconditions.checkState(
          !simplified.isEmpty(), "After simplification, OR parts should not be empty");
  ...
  private static List<Predicate> dedup(List<Predicate> children, boolean isOr) {
    Preconditions.checkArgument(
        !children.isEmpty(), "Children list should not be empty for deduplication");
    // 先递归简化子项，确保内部是干净的
  ...
  private static List<Predicate> optimizeRanges(List<Predicate> parts, boolean isOr) {
    Map<Integer, List<LeafPredicate>> groups =
        parts.stream()
  ...
  private static RangeSet<Comparable<?>> toRange(LeafPredicate lp) {
    ...
    throw new UnsupportedOperationException(
        "Unsupported function for range conversion: " + lp.function());
  }
  ...
  private static List<Predicate> fromRangeSet(FieldRef origin, RangeSet<Comparable<?>> rangeSet) {
    ...
    Preconditions.checkState(
        !rangeSet.encloses(Range.all()), "RangeSet should not enclose all here");
        ...
        if (range.lowerBoundType() == BoundType.CLOSED) {
          result.add(
              new LeafPredicate(
                  ...
                  origin.index(),
                  origin.name(),
                  Collections.singletonList(range.lowerEndpoint())));
        } else {
          result.add(
              new LeafPredicate(
        ...
        if (range.upperBoundType() == BoundType.CLOSED) {
          result.add(
              new LeafPredicate(
                  ...
                  origin.index(),
                  origin.name(),
                  Collections.singletonList(range.upperEndpoint())));
        } else {
          result.add(
              new LeafPredicate(
                  LessThan.INSTANCE,
    ...
    if (equals.size() == 1) {
      result.add(
          new LeafPredicate(Equal.INSTANCE, origin.type(), origin.index(), origin.name(), equals));
    } else if (equals.size() > 1) {
      result.add(
          new LeafPredicate(In.INSTANCE, origin.type(), origin.index(), origin.name(), equals));
    }
