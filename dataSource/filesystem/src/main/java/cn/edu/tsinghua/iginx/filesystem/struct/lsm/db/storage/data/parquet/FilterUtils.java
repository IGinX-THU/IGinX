...
public class FilterUtils {
  ...
  @Nullable
  public static Predicate toPaimonPredicate(
      Filter filter, Header header, RowType projectedSchema, List<Filter> unsupportedFilters) {
  ...
  private static ValueFilter toValueFilter(ExprFilter filter) {
    ...
    if (exprA instanceof ConstantExpression && exprB instanceof BaseExpression) {
      ...
      throw new UnsupportedOperationException(
          "Unsupported expression types in ExprFilter: "
              + exprA.getClass()
              + " and "
              + exprB.getClass());
    }
    return new ValueFilter(
        baseExpr.getPathName(), filter.getOp(), new Value(constantExpr.getValue()));
  }
  ...
  @Nullable
  private static Predicate toPaimonPredicate(
      InFilter filter, Header header, RowType projectedSchema) {
    PredicateBuilder builder = new PredicateBuilder(projectedSchema);
  ...
  @Nullable
  private static Predicate toPaimonPredicate(
      ValueFilter filter, Header header, RowType projectedSchema) {
    PredicateBuilder builder = new PredicateBuilder(projectedSchema);
    ...
    switch (filter.getOp()) {
      ...
      case LIKE_AND:
        ...
        switch (classified.getKind()) {
          ...
          default:
            throw new UnsupportedOperationException(
                "Unsupported Like pattern: " + classified.getKind());
        }
      case NOT_LIKE:
      case NOT_LIKE_AND:
      default:
  ...
  private static Predicate toPaimonPredicate(
      AndFilter filter, Header header, RowType projectedSchema) {
    List<Predicate> predicates = new ArrayList<>();
    Preconditions.checkArgument(
        !filter.getChildren().isEmpty(), "AndFilter must have at least one child");
  ...
  private static Predicate toPaimonPredicate(
      OrFilter filter, Header header, RowType projectedSchema) {
    List<Predicate> predicates = new ArrayList<>();
    Preconditions.checkArgument(
        !filter.getChildren().isEmpty(), "OrFilter must have at least one child");
    for (Filter child : filter.getChildren()) {
      Predicate childPredicate = toPaimonPredicate(child, header, projectedSchema);
      predicates.add(childPredicate);
    }
  ...
  private static int getIndex(String pattern, Header header) {
    ...
    if (indices.size() > 1) {
      throw new UnsupportedOperationException(
          "Multiple paths match pattern " + pattern + ": " + indices);
    }
    ...
    predicate.visit(
        new PredicateVisitor<Void>() {
          ...
          public Void visit(LeafPredicate predicate) {
            DataField field =
                new DataField(predicate.index(), predicate.fieldName(), predicate.type());
            if (field.id() != 0) {
  ...
  @Nullable
  public static java.util.function.Predicate<InternalRow> optimizeUnsupportedFilter(
      Filter filter, Header header, InternalRow.FieldGetter[] fieldGetters) {
    try {
      switch (filter.getType()) {
        case Value:
  ...
  @Nullable
  private static java.util.function.Predicate<InternalRow> optimizeUnsupportedFilter(
      ValueFilter filter, Header header) {
    int index = getIndex(filter.getPath(), header);
    Object value = getJavaObject(header, index, filter.getValue());
    switch (filter.getOp()) {
      case LIKE:
  ...
  private static java.util.function.Predicate<InternalRow> optimizeLike(
      int index, BinaryString pattern) {
  ...
  private static java.util.function.Predicate<InternalRow> optimizeUnsupportedFilter(
      PathFilter filter, Header header, InternalRow.FieldGetter[] fieldGetters) {
    int indexA = getIndex(filter.getPathA(), header);
    int indexB = getIndex(filter.getPathB(), header);

    Function<InternalRow, Integer> cmp =
        (internalRow) -> {
