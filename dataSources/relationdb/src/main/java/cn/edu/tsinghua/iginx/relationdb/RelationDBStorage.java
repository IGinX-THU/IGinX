 ...
 * under the License.
 */
package cn.edu.tsinghua.iginx.relationdb;
...
import cn.edu.tsinghua.iginx.relationdb.query.entity.RelationDBQueryRowStream;
import cn.edu.tsinghua.iginx.relationdb.tools.DataTypeTransformer;
import cn.edu.tsinghua.iginx.relationdb.tools.FilterTransformer;
import cn.edu.tsinghua.iginx.thrift.DataType;
import cn.edu.tsinghua.iginx.utils.Pair;
import cn.edu.tsinghua.iginx.utils.StringUtils;
import cn.edu.tsinghua.iginx.relationdb.conf.mysqlConf;
import cn.edu.tsinghua.iginx.relationdb.conf.dmdbConf;
...
import com.huawei.dmdb.jdbc.DmdbConnectionPoolDataSource;
import com.mysql.cj.jdbc.MysqlConnectionPoolDataSource;

//import static cn.edu.tsinghua.iginx.relationDB.tools.Constants.*;
import static cn.edu.tsinghua.iginx.relationdb.tools.HashUtils.toHash;
import static cn.edu.tsinghua.iginx.relationdb.tools.TagKVUtils.splitFullName;
import static cn.edu.tsinghua.iginx.relationdb.tools.TagKVUtils.toFullName;

public class RelationDBStorage implements IStorage {

    private static final Logger logger = LoggerFactory.getLogger(RelationDBStorage.class);

    private final StorageEngineMeta meta;

    private final Map<String, ConnectionPoolDataSource> connectionPoolMap = new ConcurrentHashMap<>();
    
    private final Connection connection;
    
    public final RelationDBConf r;  //关系数据库配置
    
    public final String IGINX_SEPARATOR;
    public final String RELATIONDB_SEPARATOR;

    public RelationDBStorage(StorageEngineMeta meta) throws StorageInitializationException {
        this.meta = meta;
        if (!testConnection()) {
        ...
        //找到具体的关系字类并创建实例
        String engineName = meta.getStorageEngine();
        String className = engineName + "Conf";  //关系子类的配置 类名
        Class<RelationDBConf> clr = (Class<RelationDBConf>)Class.forName("cn.edu.tsinghua.iginx.relationDB.conf."+className);
        r = clr.getConstructor().newInstance();  //子类实例
        
        this.IGINX_SEPARATOR = r.IGINX_SEPARATOR;
        this.RELATIONDB_SEPARATOR = r.RELATIONDB_SEPARATOR;
        
        String connUrl = String
                .format(r.CONNECTION_URL, meta.getIp(), meta.getPort(),
                        username, password);
        try {
    ...
    private String getUrl(String databaseName) {
        Map<String, String> extraParams = meta.getExtraParams();
        String username = extraParams.getOrDefault(r.USERNAME, r.DEFAULT_USERNAME);
        String password = extraParams.getOrDefault(r.PASSWORD, r.DEFAULT_PASSWORD);
        String connUrl = String
            .format(r.CONNECTION_URL, meta.getIp(), meta.getPort(), databaseName,
                username, password);
        return connUrl;
    }

    private Connection getConnection(String databaseName) {
        if (!r.NEED_CHANGE_DB) {  //有的数据库不需要更换连接
            return connection;
        }
        if (databaseName.startsWith("dummy")) {
            return null;
        }
        String ignoredDB = r.IGNORED_DATABASE;  //需要忽略的数据库（系统自动维护，用户没有修改权限的数据库等）
        String[] ignoredDBs = ignoredDB.split(",");
        for(String db:ignoredDBs) {
        ...
        try {
            ...
            stmt.execute(String.format(r.CREATE_DATABASE_STATEMENT, databaseName));
            stmt.close();
        } catch (SQLException e) {
//            logger.info("database {} exists!", databaseName);
        ...
        try {
            ...
            Class<Object> ConnectionPoolDataSource = Class.forName(r.CONNECTION_POOL_CLASS_NAME);
            Object connectionPool = ConnectionPoolDataSource.getConstructor().newInstance();
    ...
    public TaskExecuteResult execute(StoragePhysicalTask task) {
        ...
        return new TaskExecuteResult(
            new NonExecutablePhysicalTaskException("unsupported physical task in relationdb"));
    }
                ...
                try {
                    ...
                    while (tableSet.next()) {
                        ...
                        while (columnSet.next()) {
                            ...
                            Pair<String, Map<String, String>> nameAndTags = splitFullName(columnName, r.RELATIONDB_SEPARATOR);
                            if (databaseName.startsWith(DATABASE_PREFIX)) {
                                timeseries.add(new Timeseries(
                                    tableName.replace(RELATIONDB_SEPARATOR, IGINX_SEPARATOR)
                                    + IGINX_SEPARATOR + nameAndTags.k,
                                    DataTypeTransformer.fromRelationDB(typeName),
                                    nameAndTags.v)
                                );
                            } else {
                                timeseries.add(new Timeseries(
                                    databaseName.replace(RELATIONDB_SEPARATOR, IGINX_SEPARATOR)
                                    + IGINX_SEPARATOR + tableName.replace(RELATIONDB_SEPARATOR, IGINX_SEPARATOR)
                                    + IGINX_SEPARATOR + nameAndTags.k,
                                    DataTypeTransformer.fromRelationDB(typeName),
                                    nameAndTags.v)
                                );
        ...
        for (String pattern : patterns) {
            if (pattern.equals("*") || pattern.equals("*.*")) {
                ...
                if (pattern.split("\\" + IGINX_SEPARATOR).length == 1) { // REST 查询的路径中可能不含 .
                    ...
                    tableName = pattern.substring(0, pattern.lastIndexOf(".")).replace(IGINX_SEPARATOR, RELATIONDB_SEPARATOR);
    ...
    private TaskExecuteResult executeProjectTask(Connection conn, String databaseName, Project project, Filter filter) {
        try {
            ...
            RowStream rowStream = new ClearEmptyRowStreamWrapper(
                new RelationDBQueryRowStream(resultSets, false, filter, project.getTagFilter(),r.RELATIONDB_SEPARATOR));
            conn.close();
            return new TaskExecuteResult(rowStream);
        } catch (SQLException e) {
            logger.error(e.getMessage());
            return new TaskExecuteResult(
                new PhysicalTaskExecuteFailureException("execute project task in relationdb failure", e));
        }
    }
    ...
    private TaskExecuteResult executeHistoryProjectTask(Project project, Filter filter) {
        try {
            ...
            RowStream rowStream = new ClearEmptyRowStreamWrapper(
                new RelationDBQueryRowStream(resultSets, true, filter, project.getTagFilter(), r.RELATIONDB_SEPARATOR));
            if (conn != null) {
                conn.close();
            }
            return new TaskExecuteResult(rowStream);
    ...
    private TaskExecuteResult executeInsertTask(Connection conn, String storageUnit, Insert insert) {
        ...
        if (e != null) {
            return new TaskExecuteResult(null,
                new PhysicalException("execute insert task in relationdb failure", e));
        }
    ...
    private void createOrAlterTables(Connection conn, String storageUnit, List<String> paths, List<Map<String, String>> tagsList, List<DataType> dataTypeList) {
        for (int i = 0; i < paths.size(); i++) {
            ...
            String tableName = path.substring(0, path.lastIndexOf('.')).replace(IGINX_SEPARATOR, RELATIONDB_SEPARATOR);
            String columnName = path.substring(path.lastIndexOf('.') + 1).replace(IGINX_SEPARATOR, RELATIONDB_SEPARATOR);

            try {
                Statement stmt = conn.createStatement();
                DatabaseMetaData databaseMetaData = conn.getMetaData();
                ResultSet tableSet = databaseMetaData.getTables(storageUnit, "public", tableName, new String[]{"TABLE"});
                columnName = toFullName(columnName, tags, r.RELATIONDB_SEPARATOR);
                if (!tableSet.next()) {
                    String statement = String.format(CREATE_TABLE_STATEMENT, getCompleteName(tableName), getCompleteName(columnName), DataTypeTransformer.toRelationDB(dataType));
                    ...
                    if (!columnSet.next()) {
                        String statement = String.format(ADD_COLUMN_STATEMENT, getCompleteName(tableName), getCompleteName(columnName), DataTypeTransformer.toRelationDB(dataType));
                        logger.info("[Create] execute create: {}", statement);
                        stmt.execute(statement);
                    }
                    columnSet.close();
            ...
            while (cnt < data.getTimeSize()) {
                ...
                for (int i = cnt; i < cnt + size; i++) {
                    ...
                    for (int j = 0; j < data.getPathNum(); j++) {
                        ...
                        if (firstRound) {
                            columnKeys.append(toFullName(columnName, tags, r.RELATIONDB_SEPARATOR)).append(", ");
                        }
        ...
        try {
            ...
            if (delete.getTimeRanges() == null || delete.getTimeRanges().size() == 0) {
                if (paths.size() == 1 && paths.get(0).equals("*") && delete.getTagFilter() == null) {
                    conn.close();
                    String defaultDB = r.DEFAULT_DATABASE;
                    Connection defaultConn = getConnection(defaultDB); // 正在使用的数据库无法被删除，因此需要切换到默认数据库
                    if (defaultConn != null) {
                        ...
                        return new TaskExecuteResult(new PhysicalTaskExecuteFailureException("cannot connect to database: relationdb", new SQLException()));
                    }
                } else {
            ...
            logger.error(e.getMessage());
            return new TaskExecuteResult(
                new PhysicalTaskExecuteFailureException("execute delete task in relationdb failure", e));
        }
    }
        ...
        for (Timeseries ts: timeSeries) {
            for (String path : paths) {
                if (Pattern.matches(StringUtils.reformatPath(path), ts.getPath())) {
                    ...
                    String tableName = fullPath.substring(0, fullPath.lastIndexOf('.')).replace(IGINX_SEPARATOR, RELATIONDB_SEPARATOR);
                    String columnName = toFullName(fullPath.substring(fullPath.lastIndexOf('.') + 1), ts.getTags(), r.RELATIONDB_SEPARATOR);
