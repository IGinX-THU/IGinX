package cn.edu.tsinghua.iginx.relationdb.query.entity;
...
import  static cn.edu.tsinghua.iginx.relationdb.conf.RelationDBConf.IGINX_SEPARATOR;
...
public class RelationDBQueryRowStream implements RowStream {

    private static final Logger logger = LoggerFactory.getLogger(RelationDBQueryRowStream.class);
    ...
    public RelationDBQueryRowStream(List<ResultSet> resultSets, boolean isDummy, Filter filter, TagFilter tagFilter, String RELATIONDB_SEPARATOR) throws SQLException {
        ...
        for (int i = 0; i < resultSets.size(); i++) {
            ...
            for (int j = 1; j <= resultSetMetaData.getColumnCount(); j++) {
                ...
                Pair<String, Map<String, String>> namesAndTags = splitFullName(columnName,RELATIONDB_SEPARATOR);
                Field field = new Field(
                    tableName.replace(RELATIONDB_SEPARATOR, IGINX_SEPARATOR) + IGINX_SEPARATOR
                        + namesAndTags.k.replace(RELATIONDB_SEPARATOR, IGINX_SEPARATOR),
