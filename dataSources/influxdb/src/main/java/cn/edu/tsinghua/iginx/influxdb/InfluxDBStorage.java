...
public class InfluxDBStorage implements IStorage {
    ...
    if (op.getType() == OperatorType.Project) { // 目前只实现 project 操作符
      ...
      return isDummyStorageUnit
          ? executeHistoryProjectTask(
              task.getTargetFragment().getTsInterval(), fragment.getTimeInterval(), project)
          : executeProjectTask(
              fragment.getTimeInterval(), fragment.getTsInterval(), storageUnit, project);
  ...
  private TaskExecuteResult executeProjectTask(
      TimeInterval timeInterval, TimeSeriesRange tsInterval, String storageUnit, Project project) {
  ...
  private static String generateQueryStatement(
      String bucketName, List<String> paths, TagFilter tagFilter, long startTime, long endTime) {
    ...
    if (paths.size() != 1 || !paths.get(0).equals("*")) {
      ...
      for (int i = 0; i < paths.size(); i++) {
        ...
        filterStr.append(
            schema.getMeasurement().equals("*")
                ? "r._measurement =~ /.*/"
                : "r._measurement == \"" + schema.getMeasurement() + "\"");
        filterStr.append(" and ");
        filterStr
            .append("r._field =~ /")
            .append(InfluxDBSchema.transformField(schema.getField()))
            .append("/");
      ...
      synchronized (this) {
        ...
        if (bucket == null) {
          List<Bucket> bucketList =
              client
                  .getBucketsApi()
                  .findBucketsByOrgName(this.organizationName)
                  .stream()
          ...
          switch (data.getDataType(j)) {
            ...
            case INTEGER:
              points.add(
                  Point.measurement(schema.getMeasurement())
                      .addTags(schema.getTags())
                      .addField(schema.getField(), (int) data.getValue(i, index))
                      .time(data.getKey(i), WRITE_PRECISION));
              break;
            case LONG:
              points.add(
                  Point.measurement(schema.getMeasurement())
                      .addTags(schema.getTags())
                      .addField(schema.getField(), (long) data.getValue(i, index))
                      .time(data.getKey(i), WRITE_PRECISION));
              break;
            case FLOAT:
              points.add(
                  Point.measurement(schema.getMeasurement())
                      .addTags(schema.getTags())
                      .addField(schema.getField(), (float) data.getValue(i, index))
                      .time(data.getKey(i), WRITE_PRECISION));
              break;
            case DOUBLE:
              points.add(
                  Point.measurement(schema.getMeasurement())
                      .addTags(schema.getTags())
                      .addField(schema.getField(), (double) data.getValue(i, index))
                      .time(data.getKey(i), WRITE_PRECISION));
              break;
            case BINARY:
              points.add(
        ...
        client
            ...
            .delete(
                OffsetDateTime.ofInstant(
                    Instant.ofEpochMilli(timeRange.getActualBeginTime()), ZoneId.of("UTC")),
                OffsetDateTime.ofInstant(
                    Instant.ofEpochMilli(timeRange.getActualEndTime()), ZoneId.of("UTC")),
