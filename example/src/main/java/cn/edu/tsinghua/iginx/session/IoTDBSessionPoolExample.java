...
public class IoTDBSessionPoolExample {
  ...
  private static void downsampleQuery() throws SessionException, ExecutionException {
    ...
    SessionQueryDataSet dataSet =
        sessionPool.downsampleQuery(paths, startTime, endTime, AggregateType.MAX, INTERVAL * 100L);
    dataSet.print();

    // MIN
    dataSet =
        sessionPool.downsampleQuery(paths, startTime, endTime, AggregateType.MIN, INTERVAL * 100L);
    dataSet.print();
    ...
    // SUM
    dataSet =
        sessionPool.downsampleQuery(paths, startTime, endTime, AggregateType.SUM, INTERVAL * 100L);
    dataSet.print();

    // AVG
    dataSet =
        sessionPool.downsampleQuery(paths, startTime, endTime, AggregateType.AVG, INTERVAL * 100L);
    dataSet.print();
