...
public class InfluxDBSessionExample {
  ...
  private static void downsampleQuery() throws SessionException, ExecutionException {
    ...
    SessionQueryDataSet dataSet =
        session.downsampleQuery(paths, startTime, endTime, AggregateType.MAX, INTERVAL * 100L);
    dataSet.print();

    // MIN
    dataSet =
        session.downsampleQuery(paths, startTime, endTime, AggregateType.MIN, INTERVAL * 100L);
    dataSet.print();
    ...
    // COUNT
    dataSet =
        session.downsampleQuery(paths, startTime, endTime, AggregateType.COUNT, INTERVAL * 100L);
    dataSet.print();

    // SUM
    dataSet =
        session.downsampleQuery(paths, startTime, endTime, AggregateType.SUM, INTERVAL * 100L);
    dataSet.print();

    // AVG
    dataSet =
        session.downsampleQuery(paths, startTime, endTime, AggregateType.AVG, INTERVAL * 100L);
    dataSet.print();
